...
app = FastAPI(
    title="Trade Alert Webhook Server v2",
    description="Clean service layer architecture for processing Gmail Pub/Sub trade alerts",
    version=get_version()
)
...
async def startup_event():
    ...
    logger.info(f"🚀 Starting Trade Alert Webhook Server v2 - v{get_version()}")
...
async def shutdown_event():
    ...
    logger.info("🛑 Shutting down Trade Alert Webhook Server v2")
...
async def root():
    ...
    return {
        "service": "Trade Alert Webhook Server v2",
        "version": get_version(),
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "timestamp": datetime.utcnow().isoformat(),
        "service": "trade-alert-webhook-v2",
        "version": get_version(),
        "architecture": "service_layer"
    }
...
def run_server():
    ...
    logger.info(f"🌐 Starting webhook server v2 on {HOST}:{PORT}")
    uvicorn.run(
        "tradeflow.web.server_v2:app",
        host=HOST,
        port=PORT,
        reload=DEBUG,
